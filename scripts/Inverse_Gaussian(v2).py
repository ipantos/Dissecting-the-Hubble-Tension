...
ladder_indep_data = parse_latex_table(latex_data_ladder_indep)

print(f"Parsed {len(ladder_data)} ladder measurements")
print(f"Parsed {len(ladder_indep_data)} ladder-indep measurements")
...
mu_ladder_indep, sigma_ladder_indep, vals_ladder_indep, errs_ladder_indep = get_weighted_stats(ladder_indep_data)
...
chi2_ladder_indep, dof_ladder_indep, chi2dof_ladder_indep = calc_chi2_dof(vals_ladder_indep, errs_ladder_indep, mu_ladder_indep)

# Rounding for display
mu_ladder_r = round(mu_ladder, 2)
sigma_ladder_r = round(sigma_ladder, 2)
mu_ladder_indep_r = round(mu_ladder_indep, 2)
sigma_ladder_indep_r = round(sigma_ladder_indep, 2)
chi2dof_ladder_r = round(chi2dof_ladder, 2)
chi2dof_ladder_indep_r = round(chi2dof_ladder_indep, 2)

# Tension
delta_r = abs(mu_ladder_r - mu_ladder_indep_r)
sigma_diff_r = np.sqrt(sigma_ladder_r**2 + sigma_ladder_indep_r**2)
z_score_display = delta_r / sigma_diff_r

print(f"\n=== RESULTS ===")
print(f"Ladder: H0 = {mu_ladder_r} ± {sigma_ladder_r:.2f}")
print(f"Ladder-Indep: H0 = {mu_ladder_indep_r} ± {sigma_ladder_indep_r}")
...
y_ladder_indep_pdf = generate_weighted_pdf(ladder_indep_data, x_grid)
...
ax.plot(x_grid, y_ladder_indep_pdf, color='#A52A2A', lw=2, label='Ladder-Indep – No sound horizon')
...
# Annotations - top right (with formatted error values)
ax.text(0.97, 0.97, f"$H_0^{{\\mathrm{{dl}}}} = {mu_ladder_r} \\pm {sigma_ladder_r:.2f}$ ($\\chi^2$/dof = {chi2dof_ladder_r})",
        transform=ax.transAxes, ha='right', va='top', fontsize=11, color='#000066')
ax.text(0.97, 0.91, f"$H_0^{{\\mathrm{{li}}}} = {mu_ladder_indep_r} \\pm {sigma_ladder_indep_r}$ ($\\chi^2$/dof = {chi2dof_ladder_indep_r})",
...
ax.set_title(r'Inverse-Weighted Sum of Gaussians: Distance Ladder vs Ladder-Indep')
ax.set_xlim(55, 86)
ax.set_ylim(0, max(y_ladder_pdf.max(), y_ladder_indep_pdf.max()) * 1.15)
